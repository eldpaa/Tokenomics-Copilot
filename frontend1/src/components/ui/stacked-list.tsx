...
const MemberItem = ({ member }: { member: Member }) => (
  <motion.div
    variants={{
      hidden: { opacity: 0, transform: "translateX(10px) translateY(15px) rotate(1deg)" },
      visible: { opacity: 1, transform: "translateX(0px) translateY(0px) rotate(0deg)" },
    }}
    ...
    <div className="flex items-center justify-center min-h-screen w-full bg-muted/50 p-6 font-sans not-prose">
      <div className="relative w-full max-w-[440px] pb-6 bg-background rounded-[40px] border border-border flex flex-col overflow-hidden shadow-none">
        ...
        <motion.div
          ...
          initial={false}
          className={cn(
            "absolute z-50 overflow-hidden border border-border shadow-none flex flex-col group/bar bg-card",
            isExpanded
              ? "bottom-[10px] left-[10px] w-[calc(100%-20px)] h-[calc(100%-20px)] rounded-[32px]"
              : "bottom-[20px] left-[20px] w-[calc(100%-40px)] h-[68px] rounded-[24px]"
          )}
          transition={{
            type: "spring",
            stiffness: 240,
